  ...
  const field = (key, label, placeholder, type = 'text') => (
    <div>
      ...
      <input
        ...
        className={`input-field ${errors[key] ? 'border-rose-500/60 focus:border-rose-500' : ''}`}
      />
    ...
    <div className="card">
      <div className="card-header">
        <div className="flex items-center gap-3">
          <div className="step-indicator bg-gold-500/20 text-gold-400 border border-gold-500/30">1</div>
          <div>
            <h2 className="section-title">Student Information</h2>
            <p className="text-xs text-ink-500 mt-0.5">Enter the student details for the bill header</p>
          </div>
        </div>
      </div>
      <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
        {field('name', 'Full Name', 'e.g. something')}
        {field('id', 'Student ID', 'e.g. 2311249')}
        {field('phoneNumber', 'Phone Number', 'e.g. 01X-XXXXXXX')}
        {field('bankaccnum', 'Bank Account Number', 'e.g. 1234567890')}
